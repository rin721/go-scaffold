...
- 项目：go-scaffold
- 状态：IN_DEVELOPMENT_NOT_RELEASE_READY
- 最后更新：2026-05-27
- 路线：治理优先
...
- 目标：确认 v1 是否、何时、以什么功能和质量门禁发布。
- 状态：PENDING_USER_CONFIRMATION
- 来源：用户 2026-05-27 纠正当前项目还未开发完整，不应该发布第一版。
- 进入条件：用户明确确认第一版发布目标、剩余功能范围、真实环境验证、生产迁移、密钥管理、回滚策略和验收清单。
- 约束：在 Phase 9 未完成前，不得把当前仓库标记为 v1、release-ready 或第一版已发布。

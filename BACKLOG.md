...
| BL-024 | 增加真实 CD、镜像发布或远程部署自动化 | CI/CD 首切片后续 | P2 | 远程部署 `.env` 模板已由 TASK-P2-002 完成；手动 staging 远程部署 workflow 已由 TASK-P2-003 完成；Linux Docker production 部署制品和远程 Linux 动态 env 脚本已由 TASK-P2-004 补齐但 Docker build 待具备 Docker 的环境验证；镜像发布流水线和真实运行仍需单独确认 | [PENDING_VERIFICATION] |
